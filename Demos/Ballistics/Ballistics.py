...
class BallisticsDemo:
    '''
        Class which renders a simulation of different types of ballistics being fired in a 3D space
        This demo is supposed to show Typhoon's particle system
        ---------
        properties:
            ammoRounds      - int      - numbers of rounds to be fired before cooldown 
            ammo            - list     - All ammo to be fired
            currentShotType - ShotType - Type of shots which is being fired now
    '''

    class ShotType(Enum):
        '''
            class representing the shots type available in simulation
        '''
    ...
    class AmmoRound:
        '''
            Class which represents and renders the ammo being fired
            ---------
            properties:
                particle    - Particle       - Particle which represents the center of ammo  
                type        - ShotType       - type of ammo
                startTime   - DateTime       - time from which the shot is fired (used for removing shots after time limit)
        ...
        def render(self):
            '''
                method used to render the ammo into simulation
            '''
    ...
    def fire(self):
        '''
            Adds a new ammoRound to ammo list
        '''
